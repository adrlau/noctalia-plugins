  ...
  RowLayout {
    ...
    Rectangle {
      ...
      ColumnLayout {
        ...
        Flow {
          visible: (message && message.images && message.images.length > 0 && !root.isEditing) || false
          Layout.fillWidth: true
          spacing: Style.marginS
          
          Repeater {
            model: (message && message.images) ? message.images : []
            Rectangle {
              width: Math.min(bubbleRect.Layout.maximumWidth - (root.bubblePadding * 2), 320)
              height: Math.min(400, (img.implicitWidth > 0) ? (width * (img.implicitHeight / img.implicitWidth)) : (width * 0.75))
              ...
              MouseArea {
                anchors.fill: parent
                cursorShape: Qt.PointingHandCursor
                onClicked: root.imageClicked(modelData)
              }
            }
          }
        }
